...
## ADDENDUM 2026-08-24T2145Z - V2 LAYOUT SHIPPED + V4 REVIEW GATE PASSED
SHIPPED (commits 900cea4, 679d643, 3a9b21a): docked non-modal player (no more popup), boot-resumes last channel muted w/ pointerdown-unmute + ?ch= override, Netflix home rows (Continue/Working/My Channels/12 genre rows), guide drawer (hamburger = bottom sheet absorbing old sidebar), embed mode ?embed=1&ch=<id>[&sound=1][&genre=], Working-only toggle, wall fixes.
V4 ADVERSARIAL REVIEW applied: pointerdown-unmute (P0-1 silent resume), currentId reset in stopPlayback, home-row genre cache (perf), embed forced-muted unless &sound=1, mobile drawer/player overlap rule. Deferred: windowManager inline-onclick legacy (dead code).
PIPELINE NOTE: datacenter-probe results dropped to 0/40 (origin IP blocks) -> pipeline v3 harvests DISTRIBUTED device test results instead; working set grows from real usage.
